...
This project presents a comprehensive, cloud-native platform designed to manage, provision, compile, and monitor distributed sensor networks, particularly those based on LoRaWAN. Central to the system is its semantic data architecture: sensor metadata and measurements are modeled using Linked Data principles and stored in an RDF triplestore, enabling rich semantic queries via SPARQL. This allows flexible integration of diverse data sources, precise description of sensor attributes, and compatibility with established ontologies, laying the groundwork for interoperable, machine-readable sensor documentation.

In addition to the semantic layer, the platform integrates a modern web application, a compiler engine for firmware builds, a time-series parser, and a protobuffer-based data model. These components interact seamlessly to support the full lifecycle of IoT sensor nodes, from creation and firmware flashing to field deployment and data visualization.

Key features include a custom-built RESTful API backend, an extensible frontend built with Vue.js and Vuetify, and a compiler engine capable of enriching and compiling firmware based on per-node configuration. Two programming methods were implemented: one browser-based using the WebSerial API, and another native method using a custom python script. All sensor data and metadata are stored semantically in the RDF triplestore, while time-series measurements are additionally stored in InfluxDB for efficient querying and visualization.

The infrastructure follows modern DevOps and microservice architecture principles, using Docker for containerization and GitLab CI/CD for automated image builds. Deployment is handled via Docker Compose, with secrets managed through externalized environment variables. A reverse proxy (Caddy) provides TLS-secured access and routes requests to internal services.

The system is designed to be scalable, reproducible, and easily extendable. Although all core requirements were achieved, certain optional features, such as enhanced data visualization, CSV export, and webhook-based data ingestion, were deprioritized due to time constraints. This work demonstrates the feasibility of managing IoT sensor networks with semantic precision and a high degree of automation.
...
**Web Application:**  The web application serves as the central interface through which users interact with the system. It consists of a frontend built with Vue.js and a backend implemented using python FastAPI, providing a clear separation of concerns between presentation and business logic. The backend exposes a RESTful API consumed by the frontend to perform actions such as creating and editing entities, managing sensor nodes, and visualizing data. To ensure security and maintainability, most microservices are not directly exposed to outside. Instead, the backend selectively communicates with other internal services to delegate specific tasks, such as compiling firmware or generating data schemas.

**Triplestore:**  The triplestore serves as the central database of the system. It stores all relevant information including user accounts, sensor metadata, and the complete set of timeseries data. Write access to the triplestore is strictly controlled and limited to two internal services: the backend and the timeseries parser. Access to the triplestore is provided via a SPARQL endpoint, which is used for both internal queries and external access through the integrated SPARQL editor.

**Protobuf Service:**  The Protobuf Service encapsulates all functionality related to Protocol Buffers within the system. It provides a central interface for generating schema definitions, compiling code using the nanopb framework, and creating binary file descriptors that are essential for decoding incoming timeseries data. Communication with the service is handled exclusively through a well-defined REST interface. All relevant input and output data is transmitted via this API, and the service operates without direct access to any database or external state.

**Compiler Engine:**  The Compiler Engine is responsible for generating customized firmware binaries based on user-defined configurations. It exposes a REST interface through which requests can specify the target hardware and desired configuration parameters. To ensure that the system remains clean and resource-efficient, an auxiliary service periodically deletes old build artifacts. This setup allows for flexible, automated firmware generation while keeping the system maintainable over time.

**MQTT Broker:**  The system includes a local MQTT broker based on Eclipse Mosquitto. To meet the project’s specific requirements, a custom Docker image was created, extending the official one. This custom image allows dynamic credential configuration at runtime through environment variables – a limitation in the official image that was addressed by the team. The broker itself is configured to mirror the credentials and topic structure of the MQTT interface provided by The Things Network, enabling seamless integration with TTN’s uplink messages.

Using a local MQTT broker in addition to the external TTN broker follows recommended best practice in IoT system design. It acts as a unified internal entry point for all MQTT clients and services, allowing them to subscribe to sensor data without depending directly on external infrastructure. his architectural decision also prepares the system for integration with alternative LoRaWAN providers, as detailed in the [Future Work](#future-work) section.
...
|                                   | - Full compatibility with Arduino boards and libraries            | - Incompatible with Docker environments [@arduino-ide]             |
+-----------------------------------+-------------------------------------------------------------------+--------------------------------------------------------------------+
...
### Integration of Alternative LoRaWAN Providers
To further enhance flexibility, future work could focus on integrating alternative LoRaWAN providers. For example, providers like Swisscom Low-Power-Network (LPN) offer data delivery exclusively via Webhooks rather than MQTT. In such cases, an internal transformation service could be developed to receive the Webhook data. This service would then publish the data to the local MQTT broker in the same format as current TTN messages, ensuring the rest of the system remains unchanged and continues to interact solely with the local broker.
