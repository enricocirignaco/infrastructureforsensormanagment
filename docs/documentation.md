...
**Triplestore:** The triplestore, implemented using Apache Jena Fuseki, functions as the central database of the system. It holds all user information, sensor metadata, and the full set of timeseries data in RDF format, with each entity uniquely identified by a URI. Access is provided through a SPARQL endpoint, allowing structured queries over the semantically modeled data. Write operations are strictly limited to the backend and the timeseries parser, ensuring consistent data handling and centralized control. Fuseki was chosen for its standards compliance and smooth integration into the container-based architecture.

**Protobuf Service:** The Protobuf Service is responsible for all operations related to Protocol Buffers and encapsulates this functionality in a single, reusable component. It provides a REST interface through which schema definitions are generated, nanopb code is compiled, and binary file descriptors are produced. These descriptors are required by other services to correctly decode incoming timeseries data. By isolating protobuf logic in its own service and avoiding direct database access, the architecture remains modular, maintainable, and easy to extend.
